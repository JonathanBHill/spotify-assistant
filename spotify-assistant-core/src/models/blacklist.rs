...
impl Blacklist {
    ...
    pub fn select_artist_to_add_by_album(
        ...
        artists.iter().for_each(|(album_name, artists)| {
            ...
            // formatted_options.extend(album_artists);
        });
        // formatted_options.insert(0, "Cancel".to_string());
        let selection = dialoguer::Select::new()
...
//     fn normalize(name: String) -> String {
//         let name = name.as_str();
//         // Normalize to NFD, remove diacritical marks, and compare (case-insensitive)
//         let remove_diacritics = |s: &str| {
//             s.nfd() // Decompose
//              .filter(|c| !is_combining_mark(*c)) // Remove combining marks
//              .flat_map(char::to_lowercase) // Lowercase
//              .collect::<String>()
//         };
//
//         remove_diacritics(name)
...
//         let mut artist_vec = artists.iter().collect::<Vec<&BlacklistArtist>>();
//         artist_vec.sort_by(|a, b| a.name().cmp(&b.name()));
//         let mut artist_names = artist_vec.iter().map(|artist| {
//             artist.name()
//         }).collect::<Vec<String>>();
//         artist_names.insert(0, "Cancel".to_string());
//         let selection = dialoguer::Select::new()
...
//         }
//     }
//     pub fn select_artist_to_add_by_album(&mut self, artists_with_album: HashMap<String, Vec<SimplifiedArtist>>) -> Option<BlacklistArtist>{
//         let artists = artists_with_album;
//         // let mut artist_vec = artists.iter().collect::<Vec<&BlacklistArtist>>();
//         // artist_vec.sort_by(|a, b| a.name().cmp(&b.name()));
//         // println!("{:?}", artists);
