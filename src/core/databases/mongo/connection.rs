...
use crate::core::databases::mongo::groups::Clusters;
use crate::core::databases::mongo::models::RecommendedRecord;
use crate::core::enums::fs::ProjectFiles;
use crate::core::models::traits::MongoConnection;
...
impl Recommendations {
    pub async fn new() -> mongodb::error::Result<Self> {
        ...
        if Recommendations::test_connection(&client).await? {
            ...
            Err(mongodb::error::Error::from(std::io::Error::new(std::io::ErrorKind::Other, "Could not connect to the database")))
        }
    }
    pub async fn update_document(&self) -> mongodb::error::Result<()> {
        let coll: Collection<RecommendedRecord> = self.client.database(self.database_name).collection(self.collection_name);
        let update = coll.update_one(doc! {"nickname": "dub_trap_11_04_24"}, doc! {"$set": doc! {"generated_on": {"date": "11-04-2024"}}}).await?;
        println!("{:?} document was successfully updated", update.modified_count);
        Ok(())
    }
    pub async fn remove_document(&self, nickname: String) -> mongodb::error::Result<()> {
        ...
        println!("{:?} document was successfully removed", delete.deleted_count);
        Ok(())
    }
    pub async fn replace_document(&self, nickname: String, doc: RecommendedRecord) -> mongodb::error::Result<()> {
        let coll: Collection<RecommendedRecord> = self.client.database(self.database_name).collection(self.collection_name);
        let replace = coll.replace_one(doc! {"nickname": nickname}, doc).await?;
        println!("{:?} document was successfully replaced", replace.modified_count);
        Ok(())
    }
