...
impl UserDB {
    pub async fn new() -> Self {
        ...
        UserDB {
            ...
            explicit_filter_enabled: *user_data
                .explicit_content()
                .get("filter_enabled")
                .unwrap_or(&false),
            explicit_filter_locked: *user_data
                .explicit_content()
                .get("filter_locked")
                .unwrap_or(&false),
            last_updated: chrono::naive::NaiveDateTime::parse_from_str(&dt, "%m/%d/%Y %H:%M")
                .unwrap(),
        }
    }
}
